...
**`FUNCTION_INVOCATION_FAILED` / "could not import 'api/index.py'" right after deploy**

This means the Flask app raised while `create_app()` ran during cold start. Causes:

1. **No `DATABASE_URL` set** — the app then falls back to SQLite. Serverless
   filesystems are read-only except `/tmp`, so the fallback DB is created at
   `/tmp/edusentiai.db`. That keeps the function importable, but data in `/tmp`
   is ephemeral (wiped between cold starts) — **set `DATABASE_URL` to a real
   Postgres (Neon) for persistence.**
2. **No `SESSION_SECRET` set** — required to sign JWTs. The function will now
   still import (health and public form pages work), but admin login/auth will
   error until you set `SESSION_SECRET`.
3. **Unreachable database at cold start** — schema init (`db.create_all()`) is
   wrapped so a transient DB error no longer crashes the import; the error is
   logged and tables can be (re)created via the `/edusentiai/api/admin/seed`
   route once the DB is reachable.

After setting any missing environment variables, **redeploy** (env changes only
