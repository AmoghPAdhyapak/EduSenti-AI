...
def create_app():
    ...
    with app.app_context():
        try:
            ...
            db.create_all()
        except Exception as exc:  # pragma: no cover - cold-start resilience
            # A transient DB hiccup during a serverless cold start must NOT crash
            # the whole function import (which surfaces as FUNCTION_INVOCATION_FAILED
            # / "could not import api/index.py"). Tables can be (re)created later via
            # the /edusentiai/api/admin/seed route, and create_all() is a no-op once
            # the schema exists.
            app.logger.error("Database schema initialization skipped: %s", exc)
