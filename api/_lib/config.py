...
def _database_uri():
    ...
    if url:
        ...
        return url
    # Fallback for local dev / when no Postgres is configured. Serverless
    # filesystems (e.g. Vercel) are read-only EXCEPT for /tmp, so the SQLite
    # file must live there — otherwise db.create_all() raises
    # "sqlite3.OperationalError: unable to open database file" at startup.
    return "sqlite:////tmp/edusentiai.db"
